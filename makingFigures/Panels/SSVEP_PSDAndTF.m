...
for iCDR=1:2
    ...
    PvsFGroup = diffPvsFAllSubsTFCP(cdrGroup,:);
    tfGroup = tfDataTFCP(cdrGroup,:,:);
...
%         cla(spHandle);
        ...
        [imData,xlims,ylims,clims] = plotPColorAsImage(spHandle,timeValsTF,freqValsTF,CData,[-0.5 1.2],[],clims,colormapName);
...
for iFreq=1:length(freqAxis)
    pPvsF2(iFreq) = kruskalwallis(PvsFGroupAll(iFreq,:),cdrGroupAll,'off');
